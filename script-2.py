crossesList = [110, 125, 112, 129, 140]
noughtsList = [95, 126, 142, 143, 113]

crosses = sum(map(lambda x: 2**x,crossesList))
noughts = sum(map(lambda x: 2**x,noughtsList))
empties = 0
winsList = []
import math
    ...
    for i in look:
        if (fp + sp) & 2**i == 0:
            ...
            if opThreat[1] == 5:
                ...
                bob = checkline(line, sp + 2**i, fp + opThreat[0] - 2**line[0][0], dhashGraph[i])
                if bob == False:
                    return False
    return checkline(line[1:], sp + myThreat[0], fp, range(225))
...
'''
def ThreatSearch(first, second, depth, acc, moves, nomove):
    empty = first + second
    global winsList
    skip = nomove
    for i in moves:
        move = 2**i
        if move & empty == 0 and move & skip == 0 and depth < 8:
            threat = checkThreat(move, first, second)
            if threat[0] == 0:
                jj = checkline(acc, crosses, noughts, range(225))
                if jj:
                   print [map(lambda x: x[0], acc) + [i]]
                   winsList = winsList + [map(lambda x: x[0], acc) + [i]]
            elif threat[0] > 0:
                ac2 = acc + [[i, threat, first, second]]
                nextMoves = dhashGraph[i]
                ThreatSearch(first + move, second + threat[0], depth + 1, ac2, nextMoves, skip)
...
#print crosses
print "win's for o's:"
ThreatSearch(noughts, crosses, 0, [], range(225), 0)
print winsList
